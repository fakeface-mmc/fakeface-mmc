...
#Network based on XceptionNet
def model(img, label, phase):
    #===========ENTRY FLOW==============
    #Block 1
    net = slim.conv2d(img, 64, [3,3], stride=2, padding='valid', scope='block1_conv1') #x1을 입력 데이터로 바꾸면 됨.
    ...
    net = slim.conv2d(net, 128, [3,3], padding='valid', scope='block1_conv2')
    ...
    net = slim.separable_conv2d(net, 256, [3,3], scope='block2_dws_conv2')
    ...
    residual = slim.conv2d(net, 768, [1,1], stride=2, scope='block2_res_conv')
    #residual = slim.conv2d(net, 256, [1,1], stride=1, scope='block2_res_conv')

    residual = slim.batch_norm(residual, scope='block2_res_bn', is_training = phase)

    #Block 3
    net = tf.nn.relu(net, name='block3_relu1')
    net = slim.separable_conv2d(net, 768, [3,3], scope='block3_dws_conv1')
    net = slim.batch_norm(net, scope='block3_bn1', is_training = phase)
    net = tf.nn.relu(net, name='block3_relu2')
    net = slim.separable_conv2d(net, 768, [3,3], scope='block3_dws_conv2')
